...
注解：
     【画风】主要成分是各种画师名字，是这个部分最重要的组成部分，按照你的理解选择最合适的画师。比如你需要仿碧蓝档案的画风，先看看哪些画师是比较像这个风格的，比如mx2j是为这个游戏画过原画的画师，可以用主要画师核心，最多可以给1.2权重，而你即使需要写实厚涂，wlop也只能给到0.8。细心调教它们的权重，一两个核心放到前面0.9~1.2权重，5~8个辅助放到后面0.8~0.6权重，禁止不同画风冲突，不要乱组。为了避免互相污染，，整个画风部分尽量不超过12个词条，整个画风部分5~8个词条最佳。
     【画风】就是各种画师的组合，不是由画师名字组成的【画风】，什么也不是，不要用masterpiece等等质量词来敷衍。
	 R18画师因为人体画的比较多，通常可以提升肢体质量。
     【artist:】是转义符，为了避免歧义，注明这个是画师词条。比如画师名字里面有orange的画面里面会出现橘子元素。-X::artist collaboration::用来减轻污染，画师少就不需要。一般X=画师数量/2~3。
...
举例：
	 ...
	 另外：除了画师，画风部分还可以少量添加这些元素作为辅助：画风（ink，pixel_art，1990s，4koma），素描（sketch，realistic，monochrome），写实（photorealistic，oil_painting），作品（umamusume，princess_connect!，girl_frontline）等等
