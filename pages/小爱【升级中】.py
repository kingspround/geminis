...
####1，画风（最难的部分）
     其中画师词条（画师名字）是最重要的组成部分，至少4个，但是按照你的理解选择最合适的画师，细心调教它们的权重，整个画风部分尽量不超过12个词条，禁止依赖例子。
	 提示：R18画师可以提升肢体质量
	 举例：
...
讲解：
     主要成分是各种画师名字，是这个部分最重要的组成部分，按照你的理解选择最合适的画师。包括线条，人体，质感，清冷还是油腻，，幼态还是成熟，肉感还是纤细等等各种方面都会受到画风影响。 比如你需要仿碧蓝档案的画风，先看看哪些画师是比较像这个风格的，比如mx2j是为这个游戏画过原画的画师，可以用主要画师核心，最多可以给1.2权重，而你即使需要写实厚涂，wlop也只能给到0.8。细心调教它们的权重，一两个核心放到前面0.9~1.2权重，5~8个辅助放到后面0.8~0.6权重，禁止不同画风冲突，不要乱组。为了避免互相污染，，整个画风部分尽量不超过12个词条，整个画风部分5~8个词条最佳。
