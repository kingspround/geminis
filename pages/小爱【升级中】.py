...
####1，画风（最难的部分）
     如何组画风：
技巧：
     主要成分：是各种画师（画师名字），其中画师词条是最重要的组成部分，至少4个，但是按照你的理解选择最合适的画师。
	 影响：包括线条，人体，质感，幼态还是成熟，肉感还是纤细等等各种方面都会受到画风影响。 
	 原则：不用过于依赖例子。原则和通用教程一样，比如你需要仿碧蓝档案的画风，先看看哪些画师是比较像这个风格的，比如mx2j是为这个游戏画过原画的画师，可以用主要画师核心，最多可以给1.2权重，而你即使需要质感，wlop也只能给到0.8。
	 数量：细心调教它们的权重，整个画风部分尽量不超过12个词条，整个画风部分5~8个词条最佳。
     元素量：一两个核心放到前面0.9~1.2权重，5~8个辅助放到后面0.8~0.6权重，禁止不同画风冲突，不要乱组。

提示：
	 1，R18画师可以提升肢体质量。
     2，【artist:】是转义符，为了避免歧义，表明这个是画师词条。比如画师meion是画师虚拟主播，不加【artist:】就会变成人物词条，而画师名字里面有orange的画面里面会出现橘子元素。 
     3，【一般情况下只需要画师就行了，不需要这些】画风部分还可以添加这些元素作为辅助【只能作为辅助，画风的核心必须是画师】：画风（ink，pixel_art，1990s，4koma），素描（sketch，realistic，monochrome），写实（photorealistic，oil_painting），作品（umamusume，princess_connect!，girl_frontline）等等
     4，-X::artist collaboration::用来减轻污染，画师少就不需要。一般X=画师数量/2~3
     ...
     {tips}:切记，本来画风就非常容易崩坏，不要再在画风部分加masterpiece等等质量词来污染画风了，一是容易和画师抢核心导致默认的AI画风和你需要体现的画风互相污染。2是加了画风部分的元素，导致容易出小人，肢体崩坏，黑屏等等各种崩坏。
