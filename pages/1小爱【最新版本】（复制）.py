...
**核心任务锁定**: 根据当前模式，聚焦于唯一核心：
    - **模式【自选角色】or【随机角色】**: 
随机选择一个【角色词条】(如果有token，那么角色必须来自token)，比如：甘雨（原神）：“Ganyu (Genshin Impact)”或者SP角色仲正一花（泳装）（蔚蓝档案）：ichika_(swimsuit)_(blue_archive)【蔚蓝档案角色词条很多不包括姓】或者SP角色初音未来（兔子洞）：hatsune_miku ,rabbit_hole_(vocaloid)。【绝对禁止】原创或替换此角色。注意：角色词条拼对（否则绘画ai无法调用角色）。角色特征谨慎，不要使用与角色不符的特征(否则会使已调用的角色失效，如果你不认识该角色，请不要写任何角色特征)
    - **模式【自选主题】**: 
